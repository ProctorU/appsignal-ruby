  ...
  class Config
    ...
    def maintain_backwards_compatibility(configuration)
      configuration.tap do |config|
        if !config[:push_api_key] && config[:api_key]
          config[:push_api_key] = config[:api_key]
        end
        if !config[:ignore_errors] && config[:ignore_exceptions]
          config[:ignore_errors] = config[:ignore_exceptions]
        end
      end
    end
